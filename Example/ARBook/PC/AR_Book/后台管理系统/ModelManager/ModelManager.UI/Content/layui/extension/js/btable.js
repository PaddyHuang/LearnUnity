...
layui.define(['element', 'common', 'paging', 'form'], function (exports) {
    ...
    BTable.prototype.render = function () {
        ...
        if (_config.paged) {
            ...
            tpl += '<div data-type="paged" class="btable-paged" id="' + $(_config.elem)[0].id+ 'paged"></div>';
        }
        tpl += '</div>';
    ...
    function getTpl(options) {
        ...
        var tpl = '{{# if(d.Result.length>0 && d.Result!=undefined){ }}';
        tpl += '{{# layui.each(d.Result, function(index, item){ }}';
        var tds = '';
        for (var i = 0; i < columns.length; i++) {
            tds += '<td data-field="' + columns[i].field + '">{{ item.' + columns[i].field + ' }}</td>';
        }
        if (options.checkbox) {
            tds = '<td><input type="checkbox" data-item="id" value="{{ item.' + options.field + ' }}" data-id="{{ item.' + options.field + ' }}" lay-skin="primary" /></td><td>{{ (index+1) }}</td>' + tds;
        } else {
