...
namespace OpenAuth.App
{
    public class ModuleManagerApp :BaseApp<Module>
    {
        ...
        //public Mo ModuleManagerApp { get; set; }
        ...
        {
            var delOrgs = UnitWork.Find<ModuleElement>(u => ids.Contains(u.Id));

          //  var delOrgs = Repository.Find(u => ids.Contains(u.Id)).ToList();
                foreach (var org in delOrgs)
                {

                     UnitWork.Delete<ModuleElement>(org);
                 //ModuleManagerApp.Delete(org);                
                }
