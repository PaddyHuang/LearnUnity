...
public abstract class CardDeckInterface : MonoBehaviour
{
    ...
    [Tooltip("How the card is sorted")] public SORTING_MODE sortingMode = SORTING_MODE.DESCENDING;
    ...
    [Tooltip("Spacing between cards")] public float cardSpacing = 1.0f;

    // Intro duration in Seconds
    [Tooltip("Duration of intro animation")] [SerializeField]
    private float introDuration = 1.0f;

    // Card move speed
    [Tooltip("Card move speed")] public float cardMoveSpeed = 1.0f;

    // Starting Index
    [Tooltip("Starting card index")] [SerializeField]
    private int startingIndex = 0;

    // cards holder for inspector
    [Tooltip("Put your cards here")] [SerializeField]
    ...
    private void ListenInput()
    {
        ...
        if (Input.touchCount > 0)
        {
            ...
            if (touch.phase == TouchPhase.Moved)
            {
                ...
                if (!loop)
                {
                    if (offsett > (_cards.Count - 1))
                        offsett = _cards.Count - 1;
                    if (offsett < 0)
                        offsett = 0;
                }
